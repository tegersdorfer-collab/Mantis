...
- **Not‑Aus**: Datei `~/.mantis-forge-stop` existiert (z. B. per `touch` oder durch Fehler‑Spirale). Der Daemon sieht sie sofort und stoppt mit Exit 0, ohne laufenden Tick abzuschließen. Für sofortigen Abbruch, z. B. bei hängendem Daemon.
- **Wann was**: Verwende Halt, wenn du das Ende der aktuellen Nacht abwarten willst. Verwende Not‑Aus, wenn der Daemon nicht mehr reagiert oder du einen Notfall hast.
