...
Jev ist ein optionales Entscheidungsmodell für mehrdeutige Fälle (Werkzeugauswahl, Sprecher-Adressierung, Anspruchs-Prüfung) und ergänzt das lokale Modell statt es zu ersetzen. Der Produktionspfad läuft direkt über TypeSafe: `JEV_ENABLED=true`, `JEV_PROVIDER=typesafe` und `TYPESAFE_API_KEY` in `.env`; `JEV_MODEL=jev-latest` ist der Default. OpenRouter bleibt für Vergleichsläufe verfügbar (`JEV_PROVIDER=openrouter` + `OPENROUTER_API_KEY`). Ohne den passenden Key läuft Mantis unverändert lokal weiter. Alle Fragen und Schwellenwerte stehen gebündelt in `core/decisions.py`, der zugehörige Benchmark liegt in `bench/jev/`. `JEV_ENABLED` ist bewusst unabhängig von `LLM_LOCAL_ONLY` — die Cloud-Entscheidungen brauchen eine explizite eigene Zustimmung. Nach jeder Änderung an der `.env` muss Mantis neu gestartet werden, damit sie wirkt.
